...
```{r}
smartseq2 <- smartseq2[grep("^ERCC-", rownames(smartseq2), invert = TRUE), ]
dim(smartseq2)
```
...
smartseq2$mouse_id <- sapply(smartseq2$mouse_id, function(i) {
    ...
    paste(unlist(sapply(c(1, 3, 2), function(i) {
        paste(unique(unlist(lapply(tmp, function(x) x[i]))), collapse = "/")
    })), collapse = "-")
})
```
